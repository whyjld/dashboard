...
bool OGLContext::s_ContextErrorOccurred = false;
int OGLContext::ContextErrorHandler(Display *dpy, XErrorEvent *ev)
{
 ...
 : m_Context(0)
{
	...
	int (*oldHandler)(Display*, XErrorEvent*) = XSetErrorHandler(&OGLContext::ContextErrorHandler);
...
Window OGLContext::Createwindow(Display* display, GLXFBConfig& config)
{
	int visual_attribs[] =
	{
...
void OGLContext::Destroywindow(Window window, Colormap colormap, Display* display)
{
